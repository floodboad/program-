          ...
          <TableAction
            :actions="[
              ...
              },
              {
                ...
                color: 'error',
                ifShow: record.adjustStatus === '01',
                popConfirm: {
                  title: '是否已调整完成，确认后提交审核将不允许修改！',
                  confirm: handleMeetingAudit.bind(null, record),
                },
              },
            ]"
...
<script lang="ts">
  ...
  import { columns, searchFormSchemaind } from './pmProject.data';
  ...
  import { commitPlanAdjustAudit } from '/@/api/learnpm/pm/pmProjectMeetingPlanning';
    ...
    setup(_, { emit }) {
      ...
      const [registerTable, { reload, getSelectRowKeys, getSelectRows,setSelectedRowKeys }] = useTable({
        ...
        function handleMeetingAudit(record: Recordable, e: Event){
            e?.stopPropagation();
            try{
                commitPlanAdjustAudit(record.id)
                    .then(res => {
                        console.log(res, 'res')
      ...
      function handleView(record: Recordable, e: Event) {
        e?.stopPropagation();
         replace({
          name: '项目信息维护',
          params: { type: ActionEnum.VIEW, id: record.id ,busType:'adjust'},
        });
      }
      ...
      function handleEdit(record: Recordable, e: Event) {
       ...
       replace({
          name: '项目信息维护',
          params: { type: ActionEnum.ADJUST, id: record.id,busType:'adjust' },
        });
      }
// 弹出编辑页面
